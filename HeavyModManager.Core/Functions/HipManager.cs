...
using HeavyModManager.Core.Classes.Assets;
using HipHopFile;
...
{
    private static HipHopFile.Game Game;
    private static HipHopFile.Platform Platform;
    ...
    public static void MergeInto(string source, string destination)
    {
        ...
        Endianness = Platform == HipHopFile.Platform.GameCube ? Endianness.Big : Endianness.Little;
            ...
            switch (sourceAHDR.assetType)
            {
                ...
                case AssetType.SoundInfo:
                {
                    ...
                    if (existingAssets.Any())
                    {
                        ...
                        if (Platform == HipHopFile.Platform.GameCube)
                        {
...
{
    private HipHopFile.Game game;

    public LayerComparer(HipHopFile.Game game)
    {
        this.game = game;
    }
    ...
    public int Compare(int l1, int l2)
    {
        ...
        if (game == HipHopFile.Game.Scooby && layerOrderBFBB.Contains(l1) && layerOrderBFBB.Contains(l2))
            return layerOrderBFBB.IndexOf(l1) > layerOrderBFBB.IndexOf(l2) ? 1 : -1;

        if (game == HipHopFile.Game.BFBB && layerOrderBFBB.Contains(l1) && layerOrderBFBB.Contains(l2))
            return layerOrderBFBB.IndexOf(l1) > layerOrderBFBB.IndexOf(l2) ? 1 : -1;
