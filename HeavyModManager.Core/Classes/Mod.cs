...
public class Mod
{
    ...
    public string SaveModJson(bool isEditing)
    {
        ...
        File.WriteAllText(modJsonPath, JsonSerializer.Serialize(this, new JsonSerializerOptions { WriteIndented = true }));
    ...
    public void RemoveRemoveFiles()
    {
        ...
        {
            foreach (var rawPath in RemoveFiles.Split('\n'))
            {
                ...
                var file = Path.Combine(ModManager.GameGameFilesPath, normalized);
    ...
    {
        var mergeFilesList = (MergeFiles ?? "")
            .Split('\n')
            .Select(s => s.Trim().Replace('\\', '/').ToLowerInvariant())
        ...
        void CopyDirectory(string path)
        {
            if (Directory.Exists(path))
            {
                ...
                foreach (var file in Directory.GetFiles(path))
                {
                    ...
                    var normalizedRel = relativePath.Replace('\\', '/').ToLowerInvariant();

                    if (mergeFilesList.Contains(normalizedRel) && File.Exists(destinationFilePath))
                    {
    ...
    public bool ApplyIPSPatch(ref byte[] dol)
    {
        if (string.IsNullOrEmpty(IpsPatchBase64))
            return false;
        IPSFile ips = new(Convert.FromBase64String(IpsPatchBase64));
